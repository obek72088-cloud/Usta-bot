...
async def start(m: Message):
    ...
    await m.answer(
        "👋 <b>USTA BOT</b>\n\n"
        "O‘z hududingizdagi ustalarni toping yoki o‘zingizni bepul usta sifatida qo‘shing.",
...
async def about(c: CallbackQuery):
    await c.message.edit_text(
        ...
        "Keyinchalik qo‘shimcha PRO funksiyalar ishga tushiriladi.",
...
async def join(c: CallbackQuery, state: FSMContext):
    await state.set_state(RegisterMaster.name)
    await c.message.edit_text("1/8 👤 Ismingizni yozing:")
    await c.answer()

@dp.message(RegisterMaster.name)
async def r_name(m,state):
    await state.update_data(name=m.text); await state.set_state(RegisterMaster.phone)
    await m.answer("2/8 📞 Telefon raqamingizni yuboring (masalan +998901234567):")

@dp.message(RegisterMaster.phone)
async def r_phone(m,state):
    await state.update_data(phone=m.text); await state.set_state(RegisterMaster.category)
    await m.answer("3/8 🔧 Xizmatingizni yozing:\nMasalan: Santexnik, Elektrik, Konditsioner")

@dp.message(RegisterMaster.category)
async def r_cat(m,state):
    await state.update_data(category=m.text); await state.set_state(RegisterMaster.city)
    await m.answer("4/8 📍 Shahar/tumanni yozing:")

@dp.message(RegisterMaster.city)
async def r_city(m,state):
    await state.update_data(city=m.text); await state.set_state(RegisterMaster.district)
    await m.answer("5/8 📍 Mahalla yoki hududni yozing:")

@dp.message(RegisterMaster.district)
async def r_dist(m,state):
    await state.update_data(district=m.text); await state.set_state(RegisterMaster.experience)
    await m.answer("6/8 👨‍🔧 Tajribangiz necha yil?")
...
async def r_exp(m,state):
    ...
    await state.update_data(experience=int(m.text)); await state.set_state(RegisterMaster.description)
    await m.answer("7/8 📝 O‘zingiz va xizmatlaringiz haqida qisqacha yozing:")

@dp.message(RegisterMaster.description)
async def r_desc(m,state):
    await state.update_data(description=m.text); await state.set_state(RegisterMaster.photo)
    await m.answer("8/8 📷 Ishingizdan 1 ta rasm yuboring yoki /skip yozing.")
...
async def save_master(m,state):
    ...
    await m.answer("✅ Profilingiz muvaffaqiyatli qo‘shildi!\n\nHozircha xizmat bepul.", reply_markup=main_kb())

@dp.callback_query(F.data=="search")
async def search(c,state):
    await state.set_state(SearchMaster.category)
    await c.message.edit_text("🔎 Qaysi usta kerak?\n\nMasalan: santexnik, elektrik, konditsioner")
    await c.answer()

@dp.message(SearchMaster.category)
async def s_cat(m,state):
    await state.update_data(category=m.text); await state.set_state(SearchMaster.city)
    await m.answer("📍 Qaysi shahar/tuman?")

@dp.message(SearchMaster.city)
async def s_city(m,state):
    await state.update_data(city=m.text); await state.set_state(SearchMaster.district)
    await m.answer("📍 Mahalla/hudud (yoki 'farqi yo‘q'):")

@dp.message(SearchMaster.district)
async def s_dist(m,state):
    d=await state.get_data(); district=m.text
    con=db()
    rows=con.execute("""SELECT * FROM masters WHERE lower(category) LIKE lower(?)
        AND lower(city) LIKE lower(?) AND (lower(district) LIKE lower(?) OR ?='farqi yo‘q')
        ORDER BY verified DESC, rating DESC, reviews DESC LIMIT 10""",
        (f"%{d['category']}%",f"%{d['city']}%",f"%{district}%",district.lower())).fetchall()
    con.close(); await state.clear()
    if not rows:
        return await m.answer("😕 Hozircha mos usta topilmadi.", reply_markup=main_kb())
    for r in rows:
        text=f"👨‍🔧 <b>{r['name']}</b>\n🔧 {r['category']}\n📍 {r['city']}, {r['district']}\n⭐ {r['rating']:.1f} ({r['reviews']})\n🧰 Tajriba: {r['experience']} yil\n\n{r['description']}"
        kb=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📞 Telefon", callback_data=f"phone:{r['id']}")]
        ])
        if r["photo"]:
...
async def phone(c):
    mid=int(c.data.split(":")[1]); con=db()
    r=con.execute("SELECT phone FROM masters WHERE id=?",(mid,)).fetchone(); con.close()
